...
with the exceptions being: `signature` and `use_custom_keys`, with the former specifying
how the API was called (Unify Python Client, NodeJS client, Console etc.) and the latter
specifying whether to use custom keys or the unified keys with the provider.
...
For example, Anthropic exposes the `top_k` argument, which isn't provided by OpenAI.
If you include this argument, we will send it straight to the model.
