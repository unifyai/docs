...
client = OpenAI()
client.chat.completions.create = unify.log(client.chat.completions.create, endpoint="gpt-4o_oai@external")
...
import unify
import ollama
ollama.chat = unify.log(ollama.chat, endpoint="llama-3.1-8b-chat_ollama@external")
...
With using Unify or another client, LLM queries can be given any number of custom tags,
via the `tags` argument. See the chat completions
...
import unify
import datetime
start_time = datetime.now() - datetime.timedelta(weeks=1)
