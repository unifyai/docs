from data_flywheel.teaching_assistant import evaluatefrom data_flywheel.teaching_assistant import evaluatefrom data_flywheel.teaching_assistant import questions---
...
We can then run our evals as follows, note that the questions are passed into the agent
in batch (not one at a time), which as mentioned above, will make the execution **much**
faster:
...
The above code takes roughly ~30s to run. Things are this slow because each LLM query is
being run *serially*. The simplest way to speed up our evals is to use
...
caching makes it even faster (roughly 0.005s, or **6000x** faster), with each parallel
thread no longer needing to wait for the LLM response.
