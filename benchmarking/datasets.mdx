...
Firstly, we cover topics focused on local development with datasets:
[Creation](#creation), [Indexing and Iterating](#indexing-and-iterating),
[Combining](#combining) and [Trimming](#trimming). This covers everything you need to
know for purely local development with datasets, which can be great for initial
experimentation.
...
print(dataset) # these changes are reflected locally
```
...
console to mirror your local "ground-truth" dataset exactly at all times, when simply
using the console for dynamic visualization complementing your local experimentation,
for example. This mode is not suitable if working on a shared dataset with team-mates,
as your local version will continually overwrite any additions they've pushed upstream!
