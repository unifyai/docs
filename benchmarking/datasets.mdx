...
Datasets are basically a collection of
...
with extra fields optionally also included, such as reference answers etc.
...
Firstly, we cover topics focused on local dataset development:
[Creation](#creation), [Indexing and Iterating](#indexing-and-iterating),
[Combining](#combining), [Trimming](#trimming) and [One-Liners](#one-liners).
This covers everything you need to know to get started with some initial
...
Strictly speaking, each dataset entry is a pydantic
[Datum](https://github.com/unifyai/unify/blob/b7743c6b204633c7eb77a65a1107c2031aec18a9/unify/types.py#L123)
instance, which is a container for storing any kind of data within the Unify platform,
with no model fields, but with the pydantic argument `extra=Extra.allow` enabling
*any arbitrary fields*.

For now, we will be storing prompts of type `Prompt` in our Datasets,
with some optional extra fields in some cases.
A `Datum` instance can be initialized as follows.
If a single string is passed positionally, then this is interpreted as the user message
and is automatically upcast to a `Prompt` instance during construction.
This special treatment is given to the first *optional* positional argument due the
*very common* pattern of storing prompts in `Datum` instances:
...
A prompt can also be passed explicitly, with any *optional extra fields* as well:
...
A dataset is simply a collection of `Datum` instances.
...
but as explained above, the underlying dataset will contain `Datum` instances,
...
be set as `"download"` or `"upload"` (or `"both"` which is the same as
`True`), for example:

```python
dataset = unify.Dataset.from_upstream("my_dataset")
dataset.set_auto_sync("upload")
```
...
However, any unique contents **will be preserved** on the receiving end.
For example, having mode `"download"` will add any extra data from upstream,
but it will not delete the data you have stored locally which is not stored upstream.

Going further, you might want one of your datasets (either the local one or the upstream
one) to **mirror** the other one exactly **at all times**. This is what the modes
`"local_mirrors_upstream"` and `"upstream_mirrors_local"` are for.

When `"local_mirrors_upstream"` is set, then the local dataset will follow the
upstream version exactly, meaning any local entries not present upstream will be deleted
during the sync. This is suitable in situations where you just want to test out the
"ground-truth" upstream dataset locally, whilst keeping it fully synced, but not make
any upstream changes, nor have the local version deviate from upstream at any time.

```python
import unify
dataset = unify.Dataset.from_upstream("my_dataset", auto_sync="local_mirrors_upstream")
dataset += "a" # not permitted, raises exception
# someone pushes some upstream changes, you'd like to include
print(dataset) # these changes are reflected locally, with __repr__ triggering the sync
```

Similarly, when `"upstream_mirrors_local"` is set, then the upstream dataset will
follow the local version exactly, meaning any upstream entries not present locally will
be deleted during the sync. This is suitable in situations where you want your upstream
console to mirror your local "ground-truth" dataset exactly **at all times**, when
simply using the console for dynamic visualization complementing your local
experimentation, for example. This mode is not suitable if working on a shared dataset
with team-mates, as your local version will continually overwrite any additions they've
pushed upstream!

```python
import unify
dataset = unify.Dataset.from_upstream("my_dataset", auto_sync="upstream_mirrors_local")
# you pushed some upstream changes in another session, that you don't want to keep
dataset += "a" # the other changes are removed upstream, but "a" is added
# more upstream changes are added in another session, again we don't want to keep
print(dataset) # these other changes are again deleted, and upstream mirrors local again
```