...
inside your user account. For example, the following dataset will appear in the `maths`
folder and `john_smith` sub-folder, with the dataset name `results`:

```python
import unify
unify.Dataset(["a", "b", "c"]).set_name("maths/john_smith/results").upload()
```
