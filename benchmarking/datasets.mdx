...
- training a router on such data, to improve the overall LLM performance

We will refer to our representative example of building an AI educational assistant.

### Creating Datasets

Before we start any benchmarking, we need a prompt dataset to benchmark on!

There are three main ways to extract a prompt dataset for your task,
as explained below.
...
but for some subjects we don't have very much data, such as X with only Y prompts.
...
Lets imagine we accidentally named our dataset `"english"` when uploading it,
