...
to specify the endpoint, the dataset, and the pre-configured evaluator you would like to
use, as follows:
...
If the evaluation is still running, the status code returned will be `this`.

You can get the aggregated scores across the dataset as follows:

CODE

You can also get more granular results, with per-prompt scores, as follows:
