...
Evaluators are configured using the `X` endpoint, as follows:
...
Accordingly to the [Scale AI X Leaderboard](), Y is the best model for coding.
...
The judges can be configured as follows:

CODE

### LLM Jury

Cohere showed that LLM evals are generally less biased and more accurate when several
different models from different models are average pooled (a jury rather than a judge).

For our example, we might choose to use the models X, Y and Z for computer science,
