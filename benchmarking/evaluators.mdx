...
`agent` which is being evaluated, and also includes this in the `Evaluation` instance
returned. The public `evaluate` method makes use of your custom `_evaluate` method
internally.

The public `evaluate` method *also* performs upcasting and downcasting of the inputs if
needed, ensuring that all inputs passed to `evaluate` are cast to the required type,
as per the **type hints you've added** to your `_evaluate` method. For example, the
following runs without error, despite `_evaluate` only accepting `str` inputs for
`prompt` and `response`:
...
then no automatic casting is performed, and `prompt` is passed to `_evaluate` as a
`unify.Prompt` instance, and `response` is passed as a `unify.ChatCompletion` instance.

Lets take another look at the output representation for the `Evaluation` instance
returned:
