...
Accordingly to the [Scale AI X Leaderboard](), `Claude 3.5 Sonnet` is the best model for coding.
...
url = "https://api.unify.ai/v0/evals/create"
headers = {"Authorization": f"Bearer {KEY}"}
params = {"eval_name": "computer_science_judge", "judge_models": "claude-3.5-sonnet@aws-bedrock"}
response = requests.post(url, json=params, headers=headers)
```
...
url = "https://api.unify.ai/v0/evals/create"
headers = {"Authorization": f"Bearer {KEY}"}
params = {"eval_name": "computer_science_jury", "judge_models": ["claude-3.5-sonnet@aws-bedrock", "gpt-4o@openai"]}
response = requests.post(url, json=params, headers=headers)
```
...
Computer Science system prompt: 
```
