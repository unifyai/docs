...
Let's turn on streaming mode for the table,
...
Let's filter the log timestamps so we only get those which have be logged today or later,
...
such that we're only retrieving logs from the past 2 minutes.
