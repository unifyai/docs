...
# Download Student Data for Simulation
fname = "students.json"
if not os.path.exists(fname):
    wget.download("https://raw.githubusercontent.com/unifyai/demos/refs/heads/main/ai_tutor/students.json")
with open(fname, "r") as f:
    student_data = json.load(f)

# Download Usafe Data for Simulation
fname = "usage_data.json"
if not os.path.exists(fname):
    wget.download("https://raw.githubusercontent.com/unifyai/demos/refs/heads/main/ai_tutor/data/usage_data.json")
with open(fname, "r") as f:
    usage_data = json.load(f)

# Download Raw PDF and Image Data
...
img_dir = "pdfs/GCSE_(9–1)_Mathematics"
...
while True:
    ...
    # 6) Get Image Path
    paper_img_dir = os.path.join(img_dir, question_dict["paper_id"].replace(" ", "_"), "paper/imgs")
    markscheme_img_dir = os.path.join(img_dir, question_dict["paper_id"].replace(" ", "_"), "markscheme/imgs")

    # 7) Encode images if present
    question_imgs = question_dict.get("question_imgs")
    if question_imgs:
        question_imgs = [encode_image(os.path.join(paper_img_dir, fname)) for fname in question_imgs]
    markscheme_imgs = question_dict.get("markscheme_imgs")
    if markscheme_imgs:
        markscheme_imgs = [encode_image(os.path.join(markscheme_img_dir, fname)) for fname in markscheme_imgs]

    # 8) Log the event
    with unify.Log():
        with unify.ColumnContext("student"):
            unify.log(
