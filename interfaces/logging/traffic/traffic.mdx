...
Let's recap the requirements for our application:
...
To begin with, let's assume we are beta testing the first of these three
requirements, collecting user answers but not yet marking them in.
Simply run this
[notebook]()
...
new plot tile, select the histogram type, select timestamp for the `x` axis, and then
...
quickly hack together in an interface for logging, in this case focusing on the
traffic.
