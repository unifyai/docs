...
- For using VertexAI, you would need to add,
    ...
    - `ORCHESTRA_VERTEXAI_LOCATION` (the location can also be instead passed as the `region` field in `/chat/completions`)
...
on-prem deployment, you would need to set the `UNIFY_BASE_URL` environment variable as
the address to the orchestra service. You can find it under the `networks:` field for
the `orchestra` service inside `docker-compose.yaml`. So for e.g. if the IP assigned to
`orchestra` is `10.5.0.11`, then the `UNIFY_BASE_URL` would be `http://10.5.0.11/v0`.
