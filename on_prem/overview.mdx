...
- `.env_db`: Contains the env variables for the database
...
- For using VertexAI, you would need to add,
    ...
    - `ORCHESTRA_VERTEXAI_LOCATION` (the location can also be instead passed as the `region`
    field to `/chat/completions`)
