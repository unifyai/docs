...
[Get in touch](https://calendly.com/unify-chat/general) if you would like to get access
...
- `sessionInfo.json`: The session information for the console app, it’s mainly used for
...
orchestrating everything via Docker Compose, which spins up the three containers:

- `docker-compose.yaml`: Config file for starting the Docker Compose service
...
- For using VertexAI, you would need to add,
    ...
    - `ORCHESTRA_VERTEXAI_LOCATION` (the location can also be instead passed as the
  `region` field in `/chat/completions`)
