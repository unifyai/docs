...
<Accordion title="❓ Question [8 Marks]">
...
Based on these two, it seems as though there is a small discrepency in the text-based box formatting in the question given to the agent,
...
<Accordion title="☑️ Markscheme (Not Given to Agent)">
...
Great! Our mean error has gone down from 0.8 to 0.3, we're definitely making progress 💪

Let's take a look at the traces, to ensure that the system message template has been implemented correctly,
and each LLM call had the template variables in the system message populated correctly.
