---
title: 'Create Custom Endpoint'
api: 'PUT /v0/custom_endpoint'
---
Creates a custom endpoint. This endpoint must support the OpenAI `/chat/completions`
format. To query your custom endpoint, replace your endpoint string with `<name>@custom`
when querying the unified API.

#### Authorizations

<ParamField header="Authorization" type="string" required="true">
  Bearer authentication header of the form `Bearer <token>`, where `<token>` is your auth token.
</ParamField>

#### Query Parameters

<ParamField query="name" type="string" required="true">
Alias for the custom endpoint. This will be the name used to call the endpoint.
</ParamField>

<ParamField query="url" type="string" required="true">
Base URL of the endpoint being called. Must support the OpenAI format.
</ParamField>

<ParamField query="key_name" type="string" required="true">
Name of the API key that will be passed as part of the query.
</ParamField>

<ParamField query="mdl_name" type="string | null" >
Named passed to the custom endpoint as model name. If not specified, it will default to the endpoint alias.
</ParamField>

<RequestExample>

```bash cURL
curl --request PUT \
  --url 'https://api.unify.ai/v0/custom_endpoint?name=endpoint1&url=https://api.url1.com&key_name=key1&mdl_name=llama-3.1-8b-finetuned' \
  --header 'Authorization: Bearer <UNIFY_KEY>'
```

```python Python
import requests

url = "https://api.unify.ai/v0/custom_endpoint?name=endpoint1&url=https://api.url1.com&key_name=key1&mdl_name=llama-3.1-8b-finetuned"

headers = {"Authorization": "Bearer <token>"}

response = requests.request("PUT", url, headers=headers)

print(response.text)
```

</RequestExample>
<ResponseExample>

```json 200
{
    "info": "Custom endpoint created succesfully!"
}
```

```json 404
{
    "detail": "Custom API Key not found."
}
```

```json 422
{
    "detail": [
        {
            "loc": [
                "string"
            ],
            "msg": "string",
            "type": "string"
        }
    ]
}
```

</ResponseExample>
