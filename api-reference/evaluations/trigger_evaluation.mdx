...
An optional file upload for client-side scores. The file must be in JSONL format and the prompts must match the order of the `dataset`. Each entry should include `prompt` and `score` keys, with `score` being a float between 0 and 1. The evaluation corresponding to the `evaluator` must have `client_side=True`.
</ParamField>
...
```bash cURL
curl --request POST \
  --url 'https://api.unify.ai/v0/evaluation?evaluator=eval1&dataset=dataset1&endpoint=gpt-4o-mini@openai' \
