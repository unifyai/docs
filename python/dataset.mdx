...
```python
def set_auto_sync(auto_sync: Union[bool, str]) -> Self
```
...
def from_upstream(name: str,
                  auto_sync: Union[bool, str] = False,
                  api_key: Optional[str] = None) -> Self
```
...
```python
def upload(overwrite: bool = False) -> Self
```
...
```python
def download(overwrite: bool = False) -> Self
```
...
<a id="dataset.Dataset.inplace_add"></a>
...
<a id="dataset.Dataset.inplace_sub"></a>
