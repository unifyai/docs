...
```python
exam_results = [{"name": "Jane", "passed": False}, {"name": "John", "passed": True}]
with unify.Context("Results"):
    logs = unify.create_logs(entries=exam_results)
with unify.Context("Failed"):
    unify.add_logs_to_context([l.id for l in logs if not l.entries["passed"]], "Results")
```

If the `Log` is then updated inplace, it will be reflected in all contexts.
