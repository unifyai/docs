...
All examples in the previous section have logged entries.

```python
unify.log(x=0, y={"a": [1, 2, 3]}, msg="hello", score=0.123)
```

In this example, `x`, `y`, `msg` and `score` are all **entries**.
...
By default, `unify.Entry("...")` also change the behaviour of `get_logs`, returning only the logs which include the entry.
